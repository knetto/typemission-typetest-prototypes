...
// and readable Dutch text in the spy/dossier theme.
// ═══════════════════════════════════════════════════════════════════
...
const typingCard = document.querySelector("#dossierCodePanel");
...
function setProgress(progress) {
  ...
  uploadChecks.forEach((check, index) => {
    ...
    if (isActive && !check.classList.contains("active")) {
      ...
      if (sections[index] && !sections[index].classList.contains("unlocked")) {
...
function resetTest() {
  ...
  // Reset dossier visuals
  if (docStamp) {
    docStamp.classList.remove("unlocked");
    docStamp.textContent = "CLASSIFIED";
  }
  if (codeStatus) codeStatus.textContent = "versleuteld";
  sections.forEach(s => {
    if (s) {
...
function startTest() {
  ...
  updateOverlay();
  if (codeStatus) codeStatus.textContent = "klaar";
  ...
  const codePanel = document.querySelector("#dossierCodePanel");
  if (codePanel) {
    codePanel.animate([
...
function completeTimedTest() {
  ...
  // === Dossier decryption sequence (fast) ===
  if (codeStatus) codeStatus.textContent = "ontcijferen...";

  // Step 1: Decrypt animation - sections light up fast
  ...
  // Step 2: Stamp unlocks and poster becomes ready to reveal (1200ms)
  setTimeout(() => {
    if (docStamp) {
    ...
    if (codeStatus) codeStatus.textContent = "ontcijferd!";
...
function getRank(stats) {
  ...
  if (stats.coins >= 260) return "Ontcijferaar";
  return "Rekruut";
}
